...
(function() { /**** scoping function open ****/
        ...
        var map = L.map('map',{maxZoom:19}).setView([40.813000, -73.968334], 14);
...
/* Coloring Points */
            ...
            [s_type="Early Childhood"] {marker-fill: #8064A2;} 
            [s_type="Elementary School"] {marker-fill: #f79646;}
            [s_type="Middle School"] {marker-fill: #FF66FF;}
            [s_type="High School"] {marker-fill: #00b0f0;}
            [s_type="College"] {marker-fill: #190D71;}
            [s_type="Family and Community"] {marker-fill: #00B050;}

            }
        `);
        // create CARTO layer from source and style variables
        const pointLayer = new carto.layer.Layer(pointSource, pointStyle, {
            featureClickColumns: ['site', 'the_geom', 'bin', 's_type', 'p_type', 'add', 'link', 'latitude', 'longitude', 'getdirections', 'photo']
        });
...
pointLayer.on('featureClicked', function (event) {
    ...
    content += '<h3><b>' + '<a href=' + event.data['link'] + ' target= "_blank">Learn More</a></b></h3>'
    content += '<h3><b>' + '<a href=' + event.data['getdirections'] + ' target= "_blank">Get Directions</a></b></h3>'
    content += '<hr><img src="' + event.data['photo'] + '">'  
    content += '</div>'
    sidebar.innerHTML = content;
      // opening the sidebar 
    document.getElementById("mySidenav").style.width = "calc(100vw - 42px)";
    document.getElementById("mySidenav").style.maxWidth = "560px";
    document.getElementById("openbtn").style.display = "none";
    document.getElementById("openBoundingBox").style.display = "none";
      // flyTo instead of setView animates the move
    console.log("Zoom level: " + map.getZoom());
    map.setView([event.data['latitude'], event.data['longitude']+0.0004], 19);  
//     map.setView([event.latLng.lat-0.0004,event.latLng.lng+0.0004], 19);
...
// ************************************** Filter School and Program TYPE by Checkboxes ******************************************
    ...
    function handleCheckboxChange() {
      ...
      if ((schoolType.length && programType.length)) {
        var sql = "SELECT * FROM hcz_finaldata WHERE s_type IN ("+schoolType.join(',')+") AND p_type IN ("+programType.join(',')+")";
       ...
       // populateDropDown();
        ...
        var sql_1 = "SELECT * FROM hcz_finaldata WHERE s_type IN ("+schoolType.join(',')+")";
       ...
       // populateDropDown();
        ...
        var sql_2 = "SELECT * FROM hcz_finaldata WHERE p_type IN ("+programType.join(',')+")";
  ...
  // *********************************** NEW DROPDOWN MENU **************************************************
       function populateDropDown() {
            ...
            if (typeof inputForDrop !== 'undefined' && inputForDrop !== 'null') {
              ...
              return fetch ( 
              "https://yassida.carto.com/api/v2/sql?&q=SELECT site FROM hcz_finaldata "+ inputForDrop +""
                )
            ...
            return fetch (
              ...
              //"https://vonwildsau.carto.com/api/v2/sql?&q=SELECT site FROM hcz_livedata"+inputForDrop
            )
...
var sidebar = document.querySelector('.sidebar-feature-content');  
  ...
  document.getElementById('js-select-drop').addEventListener("change", function (event) {
        ...
        return  fetch(`https://yassida.carto.com/api/v2/sql?&q=SELECT * FROM hcz_finaldata where site like '${input}'`)
...
//           this is the data for the sidebar
            ...
            content += '<h3><b>' + '<a href=' + feature['link'] + ' target= "_blank">Learn More</a></b></h3>'
            content += '<h3><b>' + '<a href=' + feature['getdirections'] + ' target= "_blank">Get Directions</a></b></h3><hr>'
          ...
          // this opens the sidebar
            document.getElementById("mySidenav").style.width = "calc(100vw - 42px)";
            document.getElementById("mySidenav").style.maxWidth = "580px";
            document.getElementById("openbtn").style.display = "none";
            document.getElementById("openBoundingBox").style.display = "none";
            });
        });
